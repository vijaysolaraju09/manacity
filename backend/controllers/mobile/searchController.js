...
exports.search = async (req, res, next) => {
    try {
        ...
        const searchTerms = buildSearchTerms(q, resolved);
        ...
        // Index recommendations (do not run migrations from controller):
        // 1) CREATE INDEX idx_products_location_deleted_available_stock ON products (location_id, deleted_at, is_available, stock_quantity);
        // 2) CREATE INDEX idx_products_name_trgm ON products USING gin (name gin_trgm_ops);
        // 3) CREATE INDEX idx_shops_location_deleted_name ON shops (location_id, deleted_at, name);
        // 4) CREATE INDEX idx_service_categories_location_active_name ON service_categories (location_id, is_active, name);
